...
#date:Thu Mar 29 15:53:22 CST 2018
use strict;
...
&print_usage unless (defined($type));
...
if ($typenum == 1){
	...
	my $parser = Spreadsheet::ParseXLSX->new();
	$datadpth=decode("utf8",$datadpth);
	$datadpth=encode("utf8",$datadpth);
	...
	for my $worksheet($workbook->worksheet(0)){
		...
		for my $row(($row_min+1) .. $row_max){
			my $cell = $worksheet->get_cell($row,$col_min);
			$sample_id = $cell? encode ('utf8', $cell->value()) :'';
			#my $cell = $worksheet->get_cell($row,$col_min+1);
			#$q30=$cell? encode ('utf8', $cell->value()) :'';
			my $cell = $worksheet->get_cell($row,$col_min+1);
			$datasize=$cell? encode ('utf8', $cell->value()) :'';
			my $cell = $worksheet->get_cell($row,$col_min+2);
			$theodepth=$cell? encode ('utf8', $cell->value()) :'';
			if($sample_id){
	...
	foreach my $sampleid(@sampleIDs){
		...
		print Q1 encode("utf8","样本编号\t样本质控\t测序质控\tQ30\n");
		...
		print T1 encode("utf8","样本编号\t$sampleid\n");
		print T1 encode("utf8","姓名\t"),"$patientname_hash{$sampleid}\n";
		print T1 encode("utf8","年龄\t$age_hash{$sampleid}\n");
		print T1 encode("utf8","性别\t"),"$gender_hash{$sampleid}\n";
		print T2 encode("utf8","样本编号\t平均测序深度\t覆盖度10x\t覆盖度20x\t覆盖度50x\t覆盖度100x\n");
		...
		while(<$fh>){
			...
			next unless $_;
			my @tmp = split/\t/,$_;
			$tmp[0] = encode("utf8","总突变数") if $tmp[0]=~/total/;
			$tmp[0] = encode("utf8","外显子区域") if $tmp[0]=~/exonic/;
			$tmp[0] = encode("utf8","内含子区域") if $tmp[0]=~/intron/;
			$tmp[0] = encode("utf8","基因间区域") if $tmp[0]=~/intergenic/;
			$tmp[0] = encode("utf8","同义点突变") if $tmp[0]=~/^synonymous/;
			$tmp[0] = encode("utf8","错义点突变") if $tmp[0]=~/^nonsynonymous/;
			$tmp[0] = encode("utf8","无义突变") if $tmp[0]=~/stopgain/;
			$tmp[0] = encode("utf8","终止密码突变") if $tmp[0]=~/stoploss/;
			$tmp[0] = encode("utf8","非移码插入") if $tmp[0]=~/nonframeshift_insertion/;
			$tmp[0] = encode("utf8","非移码缺失") if $tmp[0]=~/nonframeshift_deletion/;
			$tmp[0] = encode("utf8","移码插入") if $tmp[0]=~/frameshift_insertion/;
			$tmp[0] = encode("utf8","移码缺失") if $tmp[0]=~/frameshift_deletion/;
			$tmp[0] = encode("utf8","潜在疾病相关位点") if $tmp[0]=~/^pathogenic_site/;
			$tmp[0] = encode("utf8","COSMIC疾病相关位点") if $tmp[0]=~/COSMIC_pathogenic_site/;
			$tmp[0] = encode("utf8","ClinVar疾病相关位点") if $tmp[0]=~/Clinvar_pathogenic_site/;
		...
		while(<$fh>){	#超出20行的输出前20行，以num of harm按从小到大排，不足的全输
			chomp;
			my $line6 = t6_split_colon($_);
			push @unsorted_lines,$line6;
			#push @unsorted_lines,decode('utf8',$_);
		}
		close $fh;
		...
		#print OUT encode("utf8","化疗药物\t检测基因\t检测区域\t检测结果\t结果解读\t等级\n");
	...
	my $parser = Spreadsheet::ParseXLSX->new();
	$sampleinfo=decode("utf8",$sampleinfo);
	$sampleinfo=encode("utf8",$sampleinfo);
	#print $sampleinfo;
	my $workbook = $parser ->parse("$sampleinfo");
	...
	for my $worksheet($workbook->worksheet(0)){
		...
		$ctrlID{$sampleID} = $cell? encode ('utf8', $cell->value()) :'';
		...
		$ctrltype{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';
	...
	foreach my $sampleid(@sampleIDs){
		...
		print T1 encode("utf8","样本编号\t$sampleid\t$ctrlID{$sampleid}\n");
		#print T1 encode("utf8","姓名\t"),"$patientname_hash{$sampleid}\t$patientname_hash{$ctrlID{$sampleid}}\n";
		print T1 encode("utf8","姓名\t"),"$patientname_hash{$sampleid}\t$patientname_hash{$sampleid}\n";
		#print T1 encode("utf8","年龄\t$age_hash{$sampleid}\t$age_hash{$ctrlID{$sampleid}}\n");
		print T1 encode("utf8","年龄\t$age_hash{$sampleid}\t$age_hash{$sampleid}\n");
		#print T1 encode("utf8","性别\t"),"$gender_hash{$sampleid}\t$gender_hash{$ctrlID{$sampleid}}\n";
		print T1 encode("utf8","性别\t"),"$gender_hash{$sampleid}\t$gender_hash{$sampleid}\n";
		print T2 encode("utf8","样本编号\t平均测序深度\t覆盖度10x\t覆盖度20x\t覆盖度50x\t覆盖度100x\n");
		print T2 $sampleid,"\t$MeanDp_hash{$sampleid}\t$tenDp_hash{$sampleid}\t$twentyCvg_hash{$sampleid}\t$fiftyDp_hash{$sampleid}\t$hundredDp_hash{$sampleid}\n";
		...
		if ($ctrlID{$sampleid} =~ /^20/){
			print OUT "$ctrlID{$sampleid}\n"
		}
		else{
			print OUT "20$ctrlID{$sampleid}\n"
		}
	...
	for my $worksheet($workbook->worksheet(0)){
		...
		$ctrlreceive_date_hash{$sampleID} = $cell? encode ('utf8', $cell->value()) :'';	#对照收样日期
		my $cell = $worksheet->get_cell($row_min+2,$col_min+1);
		$ctrlID{$sampleID} = $cell? encode ('utf8', $cell->value()) :'';	#对照样本编号
		my $cell = $worksheet->get_cell($row_min+1,$col_min+2);
		$patientname=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+2);
		$ctrlname_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#对照姓名
		my $cell = $worksheet->get_cell($row_min+1,$col_min+3);
		$donorname=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+3);
		$ctrldonorname_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#对照供者姓名
		my $cell = $worksheet->get_cell($row_min+1,$col_min+4);
		$kiname=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+4);
		$ctrlkiname_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#对照家属姓名
		my $cell = $worksheet->get_cell($row_min+1,$col_min+5);
		$age=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+5);
		$ctrl_age_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#对照年龄
		my $cell = $worksheet->get_cell($row_min+1,$col_min+6);
		$gender=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+6);
		$ctrl_gender_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#对照性别
		my $cell = $worksheet->get_cell($row_min+1,$col_min+7);
		$sampletype=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+7);
		$ctrltype{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#对照样本类型
		my $cell = $worksheet->get_cell($row_min+1,$col_min+9);
		$diagnose=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+9);
		$ctrldiagnose_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#对照诊断
		my $cell = $worksheet->get_cell($row_min+1,$col_min+11);
		$hospital=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+11);
		$ctrlhospital_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#对照送检医院
		my $cell = $worksheet->get_cell($row_min+1,$col_min+14);
		$sampleQ=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+14);
		$ctrlQ{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#对照样本质控
		my $cell = $worksheet->get_cell($row_min+1,$col_min+15);
		$sequenceQ=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+15);
		$ctrlseqQ{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#对照测序质控
		my $cell = $worksheet->get_cell($row_min+1,$col_min+16);
		$Q30=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+16);
		$ctrlQ30{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#对照Q30
		if($sampleID){
			$receive_date_hash{$sampleID}= $receive_date;
			$patientname_hash{$sampleID} = $patientname;
			$donorname_hash{$sampleID}   = $donorname;
	...
	foreach my $sampleid(@sampleIDs){
		...
		print T1 encode("utf8","姓名\t"),"$patientname_hash{$sampleid}\t$altername{$sampleid}\n";
		print T1 encode("utf8","年龄\t$age_hash{$sampleid}\t$ctrl_age_hash{$sampleid}\n");
		print T1 encode("utf8","性别\t"),"$gender_hash{$sampleid}\t$ctrl_gender_hash{$sampleid}\n";
		print T2 encode("utf8","样本编号\t平均测序深度\t覆盖度10x\t覆盖度20x\t覆盖度50x\t覆盖度100x\n");
		print T2 $sampleid,"\t$MeanDp_hash{$sampleid}\t$tenDp_hash{$sampleid}\t$twentyCvg_hash{$sampleid}\t$fiftyDp_hash{$sampleid}\t$hundredDp_hash{$sampleid}\n";
		print T2 $ctrlID{$sampleid},"\t$MeanDp_hash{$ctrlID{$sampleid}}\t$tenDp_hash{$ctrlID{$sampleid}}\t$twentyCvg_hash{$ctrlID{$sampleid}}\t$fiftyDp_hash{$ctrlID{$sampleid}}\t$hundredDp_hash{$ctrlID{$sampleid}}\n";
		close INFO;
	...
	for my $worksheet($workbook->worksheet(0)){
		...
		$sampleID=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min);
		$ctrl1receive_date_hash{$sampleID} = $cell? encode ('utf8', $cell->value()) :'';	#父本收样日期
		my $cell = $worksheet->get_cell($row_min+3,$col_min);
		$ctrl2receive_date_hash{$sampleID} = $cell? encode ('utf8', $cell->value()) :'';	#母本收样日期
		my $cell = $worksheet->get_cell($row_min+2,$col_min+1);
		$ctrl1ID{$sampleID} = $cell? encode ('utf8', $cell->value()) :'';	#父本样本编号
		my $cell = $worksheet->get_cell($row_min+3,$col_min+1);
		$ctrl2ID{$sampleID} = $cell? encode ('utf8', $cell->value()) :'';	#母本样本编号
		my $cell = $worksheet->get_cell($row_min+1,$col_min+2);
		$patientname=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+2);
		$ctrl1name_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#对照姓名1
		my $cell = $worksheet->get_cell($row_min+3,$col_min+2);
		$ctrl2name_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#对照姓名2
		my $cell = $worksheet->get_cell($row_min+1,$col_min+3);
		$donorname=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+3);
		$ctrl1donorname_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#对照供者姓名1
		my $cell = $worksheet->get_cell($row_min+3,$col_min+3);
		$ctrl2donorname_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#对照供者姓名2
		my $cell = $worksheet->get_cell($row_min+1,$col_min+4);
		$kiname=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+4);
		$ctrl1kiname_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#父本姓名
		my $cell = $worksheet->get_cell($row_min+3,$col_min+4);
		$ctrl2kiname_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#母本姓名
		my $cell = $worksheet->get_cell($row_min+1,$col_min+5);
		$age=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+5);
		$ctrl1_age_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#父本年龄
		my $cell = $worksheet->get_cell($row_min+3,$col_min+5);
		$ctrl2_age_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#母本年龄
		my $cell = $worksheet->get_cell($row_min+1,$col_min+6);
		$gender=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+6);
		$ctrl1_gender_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#父本性别
		my $cell = $worksheet->get_cell($row_min+3,$col_min+6);
		$ctrl2_gender_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#母本性别
		my $cell = $worksheet->get_cell($row_min+1,$col_min+7);
		$sampletype=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+7);
		$ctrl1type{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#父本样本类型
		my $cell = $worksheet->get_cell($row_min+3,$col_min+7);
		$ctrl2type{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#母本样本类型
		my $cell = $worksheet->get_cell($row_min+1,$col_min+9);
		$diagnose=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+9);
		$ctrl1diagnose_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#父本诊断
		my $cell = $worksheet->get_cell($row_min+3,$col_min+9);
		$ctrl2diagnose_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#母本诊断
		my $cell = $worksheet->get_cell($row_min+1,$col_min+11);
		$hospital=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+11);
		$ctrl1hospital_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#父本送检医院
		my $cell = $worksheet->get_cell($row_min+3,$col_min+11);
		$ctrl2hospital_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#母本送检医院
		my $cell = $worksheet->get_cell($row_min+1,$col_min+14);
		$sampleQ=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+14);
		$ctrl1Q{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#父本样本质控
		my $cell = $worksheet->get_cell($row_min+3,$col_min+14);
		$ctrl2Q{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#母本样本质控
		my $cell = $worksheet->get_cell($row_min+1,$col_min+15);
		$sequenceQ=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+15);
		$ctrl1seqQ{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#父本测序质控
		my $cell = $worksheet->get_cell($row_min+3,$col_min+15);
		$ctrl2seqQ{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#母本测序质控
		my $cell = $worksheet->get_cell($row_min+1,$col_min+16);
		$Q30=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+16);
		$ctrl1Q30{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#父本Q30
		my $cell = $worksheet->get_cell($row_min+3,$col_min+16);
		$ctrl2Q30{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#母本Q30
		if($sampleID){
			$receive_date_hash{$sampleID}= $receive_date;
			$patientname_hash{$sampleID} = $patientname;
			$donorname_hash{$sampleID}   = $donorname;
	...
	foreach my $sampleid(@sampleIDs){
		...
		print T1 encode("utf8","样本编号\t$sampleid\t$ctrl1ID{$sampleid}\t$ctrl2ID{$sampleid}\n");
		#my %altername;
		#$altername{$sampleid} = $ctrl1name_hash{$sampleid} ? $ctrl1name_hash{$sampleid} : $ctrl1kiname_hash{$sampleid};
		print T1 encode("utf8","姓名\t"),"$patientname_hash{$sampleid}\t$ctrl1kiname_hash{$sampleid}\t$ctrl2kiname_hash{$sampleid}\n";
		print T1 encode("utf8","年龄\t$age_hash{$sampleid}\t$ctrl1_age_hash{$sampleid}\t$ctrl2_age_hash{$sampleid}\n");
		print T1 encode("utf8","性别\t"),"$gender_hash{$sampleid}\t$ctrl1_gender_hash{$sampleid}\t$ctrl2_gender_hash{$sampleid}\n";
		print T2 encode("utf8","样本编号\t平均测序深度\t覆盖度10x\t覆盖度20x\t覆盖度50x\t覆盖度100x\n");
		print T2 $sampleid,"\t$MeanDp_hash{$sampleid}\t$tenDp_hash{$sampleid}\t$twentyCvg_hash{$sampleid}\t$fiftyDp_hash{$sampleid}\t$hundredDp_hash{$sampleid}\n";
		...
		if ($ctrl1ID{$sampleid} =~ /^20/){
			print OUT "$ctrl1ID{$sampleid}\n"
		}
		else{
			print OUT "20$ctrl1ID{$sampleid}\n"
		}
		...
		if ($ctrl2ID{$sampleid} =~ /^20/){
			print OUT "$ctrl2ID{$sampleid}\n"
		}
		else{
			print OUT "20$ctrl2ID{$sampleid}\n"
		}
		...
		for(@sorted_comhs){
			print $comh_fh encode("utf8",$_);	#mutants_pedigree_ch.csv
		}
		for(@sorted_homos){
			print $homo_fh encode("utf8",$_);	#mutants_pedigree_rh.csv
		}
		for(@sorted_denovos){
			print $denovo_fh encode("utf8",$_);	#mutants_pedigree_dn.csv
		}
		close $comh_fh;
		close $homo_fh;
		close $denovo_fh;
	...
	for my $worksheet($workbook->worksheet(0)){
		...
		$sampleID=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min);
		$case2receive_date_hash{$sampleID} = $cell? encode ('utf8', $cell->value()) :'';	#case2收样日期
		my $cell = $worksheet->get_cell($row_min+3,$col_min);
		$ctrlreceive_date_hash{$sampleID} = $cell? encode ('utf8', $cell->value()) :'';	#ctrl收样日期
		my $cell = $worksheet->get_cell($row_min+2,$col_min+1);
		$case2ID{$sampleID} = $cell? encode ('utf8', $cell->value()) :'';	#case2样本编号
		my $cell = $worksheet->get_cell($row_min+3,$col_min+1);
		$ctrlID{$sampleID} = $cell? encode ('utf8', $cell->value()) :'';	#ctrl样本编号
		my $cell = $worksheet->get_cell($row_min+1,$col_min+2);
		$patientname=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+2);
		$case2name_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#case2姓名
		my $cell = $worksheet->get_cell($row_min+3,$col_min+2);
		$ctrlname_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';		#对照姓名
		my $cell = $worksheet->get_cell($row_min+1,$col_min+3);
		$donorname=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+3);
		$case2donorname_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#case2供者姓名
		my $cell = $worksheet->get_cell($row_min+3,$col_min+3);
		$ctrldonorname_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#对照供者姓名
		my $cell = $worksheet->get_cell($row_min+1,$col_min+4);
		$kiname=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+4);
		$case2kiname_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#case2家属姓名
		my $cell = $worksheet->get_cell($row_min+3,$col_min+4);
		$ctrlkiname_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#ctrl家属姓名
		my $cell = $worksheet->get_cell($row_min+1,$col_min+5);
		$age=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+5);
		$case2_age_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#case2年龄
		my $cell = $worksheet->get_cell($row_min+3,$col_min+5);
		$ctrl_age_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#ctrl年龄
		my $cell = $worksheet->get_cell($row_min+1,$col_min+6);
		$gender=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+6);
		$case2_gender_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#case2性别
		my $cell = $worksheet->get_cell($row_min+3,$col_min+6);
		$ctrl_gender_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#ctrl性别
		my $cell = $worksheet->get_cell($row_min+1,$col_min+7);
		#$sampletype=$cell? encode ('utf8', $cell->value()) :'';
		$sampletype=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+7);
		#$case2type{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#case2样本类型
		$case2type{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#case2样本类型
		my $cell = $worksheet->get_cell($row_min+3,$col_min+7);
		#$ctrltype{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#ctrl样本类型
		$ctrltype{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#case2样本类型
		my $cell = $worksheet->get_cell($row_min+1,$col_min+9);
		$diagnose=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+9);
		$case2diagnose_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#case2诊断
		my $cell = $worksheet->get_cell($row_min+3,$col_min+9);
		$ctrldiagnose_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#ctrl诊断
		my $cell = $worksheet->get_cell($row_min+1,$col_min+11);
		$hospital=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+11);
		$case2hospital_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#case2送检医院
		my $cell = $worksheet->get_cell($row_min+3,$col_min+11);
		$ctrlhospital_hash{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#ctrl送检医院
		my $cell = $worksheet->get_cell($row_min+1,$col_min+14);
		$sampleQ=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+14);
		$case2Q{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#case2样本质控
		my $cell = $worksheet->get_cell($row_min+3,$col_min+14);
		$ctrlQ{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#ctrl样本质控
		my $cell = $worksheet->get_cell($row_min+1,$col_min+15);
		$sequenceQ=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+15);
		$case2seqQ{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#case2测序质控
		my $cell = $worksheet->get_cell($row_min+3,$col_min+15);
		$ctrlseqQ{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#ctrl测序质控
		my $cell = $worksheet->get_cell($row_min+1,$col_min+16);
		$Q30=$cell? encode ('utf8', $cell->value()) :'';
		my $cell = $worksheet->get_cell($row_min+2,$col_min+16);
		$case2Q30{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#case2Q30
		my $cell = $worksheet->get_cell($row_min+3,$col_min+16);
		$ctrlQ30{$sampleID}=$cell? encode ('utf8', $cell->value()) :'';	#ctrlQ30
		if($sampleID){
			$receive_date_hash{$sampleID}= $receive_date;
			$patientname_hash{$sampleID} = $patientname;
			$donorname_hash{$sampleID}   = $donorname;
	...
	foreach my $sampleid(@sampleIDs){
		...
		print INFO "sample.type.case2\t$case2type{$sampleid}\n";
		print INFO "sample.type.ctrl\t$ctrltype{$sampleID}\n";
		...
		print T1 encode("utf8","样本编号\t$sampleid\t$case2ID{$sampleid}\t$ctrlID{$sampleid}\n");
		...
		print T1 encode("utf8","姓名\t"),"$patientname_hash{$sampleid}\t$case2_altername{$sampleid}\t$ctrl_altername{$sampleid}\n";
		print T1 encode("utf8","年龄\t$age_hash{$sampleid}\t$case2_age_hash{$sampleid}\t$ctrl_age_hash{$sampleid}\n");
		print T1 encode("utf8","性别\t"),"$gender_hash{$sampleid}\t$case2_gender_hash{$sampleid}\t$ctrl_gender_hash{$sampleid}\n";
		print T2 encode("utf8","样本编号\t平均测序深度\t覆盖度10x\t覆盖度20x\t覆盖度50x\t覆盖度100x\n");
		print T2 $sampleid,"\t$MeanDp_hash{$sampleid}\t$tenDp_hash{$sampleid}\t$twentyCvg_hash{$sampleid}\t$fiftyDp_hash{$sampleid}\t$hundredDp_hash{$sampleid}\n";
		...
		if ($case2ID{$sampleid} =~ /^20/){
			print OUT "$case2ID{$sampleid}\t";
		}
		else{
			print OUT "20$case2ID{$sampleid}\t";
		}
	...
	#########################case-case####################################
		...
		print BOTH "NA\tNA\tNA\tNA\tNA\tNA\tNA\n" if (@t4t6_lines == 0);
		close BOTH;
		#########################case-case-ctrl####################################
		...
		print SPEC "NA\tNA\tNA\tNA\tNA\tNA\tNA\n" if (@t4t6_lines == 0);
		close SPEC;
